...
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Область:";
this.input_region = L.DomUtil.create('input', '', line);
this.input_region.placeholder = "Київська";

// === НАСЕЛЕНИЙ ПУНКТ ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Нас. пункт:";
this.input_settlement = L.DomUtil.create('input', '', line);
this.input_settlement.placeholder = "Бровари";

// === ВУЛИЦЯ ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Вулиця:";
this.input_street = L.DomUtil.create('input', '', line);
this.input_street.placeholder = "Київська";

// === БУДИНОК ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Будинок:";
this.input_house = L.DomUtil.create('input', '', line);
this.input_house.placeholder = "227";

// === АЗИМУТ ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Азимут:";
this.input_azimuth = L.DomUtil.create('input', '', line);
this.input_azimuth.placeholder = "0 - 360";
this.input_azimuth.type = "number";
this.input_azimuth.min = 0;
this.input_azimuth.max = 360;
// === КОЛІР ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Колір:";
this.input_color = L.DomUtil.create('input', '', line);
this.input_color.type = "color";
this.input_color.value = "#ff0000";
// === ДАТА ТА ЧАС ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
L.DomUtil.create('span', 'tab-panel-label', line).innerHTML = "Дата/час:";
this.input_datetime = L.DomUtil.create('input', '', line);
this.input_datetime.placeholder = "01.01.2025 4:04";


// === КНОПКА ПОШУКУ ===
line = L.DomUtil.create('div', 'tab-panel-inputbar', this.inputs[0]);
var btn = L.DomUtil.create('span', 'tab-panel-button', line);
btn.innerHTML = "Знайти";
L.DomEvent.addListener(btn, 'click', function(ev){
  L.DomEvent.stop(ev);
  App.core.searchStructuredAddress();
}, this);
    ...
    line.innerHTML = 'Для того, щоб <b>Побудувати</b> адрес - почніть вводити адресу в рядок введення і виберіть відповідний з випадаючого списку підказок.';
