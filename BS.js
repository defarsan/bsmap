...
App.MapObject.extend(App.BS, {
  ...
  //  var pcolor = jQuery.Color(color).lightness(jQuery.Color(color).lightness()*1.2).toRgbaString();
      ...
      this.polygon.bindTooltip(this.getTooltipTitle(), {
       permanent: false,
       direction: 'center',
      ...
      this.setStyle({ weight: 1, fillOpacity: 0.27 });
      });
  ...
  getTooltipTitle: function () {
   ...
   var text = 'Адрес: ' + this.getTitle();
   text += '<br>Азимут: ' + this.escapeHTML(this.options.azimut);

   if (this.options.datetime) {
     text += '<br>Дата/час: ' + this.escapeHTML(this.options.datetime);
   }
